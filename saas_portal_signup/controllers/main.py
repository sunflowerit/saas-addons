...
class AuthSignupHome(auth_signup.controllers.main.AuthSignupHome):
    ...
    def web_auth_signup(self, *args, **kw):
        if not kw.get('redirect', False) and kw.get('dbname', False):
            ...
            kw['redirect'] = '%s?dbname=%s&plan_id=%s&domain=%s' % (
                redirect, kw['dbname'], kw['plan_id'], kw['domain']
            )
    ...
    def get_auth_signup_qcontext(self):
        ...
        if not qcontext.get('domains', False):
            qcontext['domains'] = request.env['saas.server'].sudo().search([])
        ...
        if not qcontext.get('base_saas_domain', False):
            qcontext['saas_domain'] = self.get_default_domain()
        return qcontext

    def get_default_domain(self):
        server = request.env['saas.server'].sudo().search([], limit=1)
        if server:
    ...
    def do_signup(self, qcontext):
        ...
        if qcontext.get('dbname', False):
            f_dbname = '%s.%s' % (qcontext['dbname'], self.get_default_domain())
